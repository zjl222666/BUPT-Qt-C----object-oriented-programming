...
void mySystem::shopping(QString comName, QString merchantsName, double price) {
    if (this->nowUser == NULL) {
        QMessageBox::information(NULL, "提醒" , "你得先登录才能购买商品哦！");
        return;
    }
    if (this->findCom(comName, merchantsName).leftNum <= 0) {
        QMessageBox::information(NULL, "提醒" , "该商品已经没有库存啦！快提醒商家补货");
        return;
    }
    if (this->nowUser->getMoney() - price < 1e-3) {
        QMessageBox::information(NULL, "提醒", "你的余额不足啦！ 快充值！");
        return;
    }
    commodityStruct tmp = this->findCom(comName, merchantsName);
    tmp.leftNum -= 1;
    this->myCommodityDatabase->updataComData(tmp);
    this->myUserDatabase->userMoneyAdd(this->nowUser->getName(), -price);
    this->myUserDatabase->userMoneyAdd(merchantsName.toStdString().c_str(), price);
    QMessageBox::information(NULL, "提醒", "购买成功！");
    return;
}
